---
title: "Intents Guide"
description: "Create, train, test, and expose conversational intents."
---

Define and train intents to route conversations to flows. Use the sections below to create intents, improve recognition, validate detection, expose intents as APIs, and configure email-specific intents.
...
<Steps>
  ...
  <Step title="Map to flow">
    Connect to a flow for handling. See [Mapping triggers to workflows](/triggers-intents/triggers-guide#mapping-triggers-to-workflows).
  </Step>
</Steps>
...
Email assistants use intents that are similar to conversational intents but differ in a few key areas: email funnel filters, email channel credentials, and graph routing.

An email intent consists of:

- **Name** – Identifies the intent.
- **Intent Phrase** – Explains the intent's purpose and utterances used to recognize user goals.
- **Funnel Down the Emails** – Basic and advanced filters.
- **Integration Funnel** – Email channel credentials to connect with.
- **Connected Agent Graph** – The graph triggered when this intent is matched.

For the full walkthrough—including create steps, AI prompt generation, and intent management—see [Intents (Email Assistants)](/concepts/overview#intents-and-triggers-email).
